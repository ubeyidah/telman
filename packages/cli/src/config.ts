...
export function writeTelegramBotToken({token, chatId}:{token: string, chatId: string}) {
  ...
  writeFileSync(configPath, JSON.stringify({ telegramBotToken: token, telegramChatId: chatId}, null, 2), {
    encoding: "utf-8",
    mode: "0o600",
  });
...
export function getTelegramBotToken() {
  if (!existsSync(configPath)) {
    throw new Error("Telegram bot token and chatId is not set. Please run 'telman init' to set it.");
  }
